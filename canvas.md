...
5行目の`ctx.fillStyle = "#FF0000";`では、「これから色を塗るときには赤色を使ってね」と設定しています。
色の指定方法については「HTML カラーコード」で検索してみてください。

6行目の`ctx.fillRect(0, 0, width, height);`で色を塗れという命令をしています。
引数を4つ取る関数なので`,`で区切って4つ指定しています。
それぞれの引数の意味はそれぞれ、塗る四角形の左上のx座標, y座標, 四角形の幅, 高さです。
